...
address 0x5a5f95171d6dfa6f0e7d41900d41fdab651560803f6937bbc081cf229c169209 {
    ...
    module NFTMarketplace {
        ...
        // TODO# 21: Analytics Dashboard Function
        #[view]
        public fun get_analytics(marketplace_addr: address): (u64, vector<(u64, u64)>, vector<(u8, u64)>) acquires Marketplace {
            let marketplace = borrow_global<Marketplace>(marketplace_addr);

            let mut total_sales = 0u64;
            let mut sales_by_nft = vector::empty<(u64, u64)>();
            let mut rarity_counts = vector::empty<(u8, u64)>();

            // Initialize rarity counters for each rarity level (assuming max rarity is 3)
            let rarity_levels = 4; // 0 (unused), 1 (common), 2 (rare), 3 (epic)
            let mut rarity_totals = vector::empty<u64>();
            let mut i = 0;
            while (i < rarity_levels) {
                vector::push_back(&mut rarity_totals, 0);
                i = i + 1;
            };
            ...
            while (j < nfts_len) {
                let nft = vector::borrow(&marketplace.nfts, j);

                // Accumulate total sales if the NFT has been sold
                if (!nft.for_sale && nft.price > 0) {
                    total_sales = total_sales + nft.price;
                    vector::push_back(&mut sales_by_nft, (nft.id, nft.price));
                };

                // Increment rarity counter
                let rarity_index = nft.rarity as u64;
                let current_count = *vector::borrow(&rarity_totals, rarity_index);
                vector::borrow_mut(&mut rarity_totals, rarity_index) = current_count + 1;

                j = j + 1;
            };

            // Convert rarity totals to (rarity, count) tuples
            let k = 1; // Start from 1 to skip unused rarity level 0
            while (k < rarity_levels) {
                vector::push_back(&mut rarity_counts, (k as u8, *vector::borrow(&rarity_totals, k)));
                k = k + 1;
            };

            (total_sales, sales_by_nft, rarity_counts)
        }
